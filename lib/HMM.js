...
var tag = curry(function rawTagSentence(outputs, transitions, sent) {
    ...
    function calc(word, fromTag, toTag) {
        ...
        var toTagProb = outputs[toTag][word] || SMOOTHING_COEFF;
...
function parse(sents) {
    ...
    _.each(sents, function parseSentence(s) {
        ...
        _.reduce(s.split(' ').concat(TAG_END + '/' + TAG_END), function parseWordAndTransition(memo, w, idx) {
            if (_.isEmpty(w)) {
                return memo;
            }
...
function smooth(outputs) {
    var smoothedOutputs = _.mapObject(outputs, function smoothWordOutputs(wordProbs) {
        ...
        wordProbs[TAG_SMOOTHED] = 10 * SMOOTHING_COEFF / (1 + wordProbs[TAG_TOTAL]);
        return wordProbs;
    });
...
function diff(expected, actual) {
    ...
    return _.map(wordDiff, function prettyDiff(diffInfo) {
        if (diffInfo.added) {
            return {correct: false, str: clc.red(diffInfo.value)};
        }
        if (diffInfo.removed) {
            return {correct: false, str: clc.green('(' + diffInfo.value + ')')};
        }
        return {correct: true, str: diffInfo.value};
    });
}
...
module.exports = function HMM(trainData, testData) {
    ...
    var actualData, testResults, smoothedOutputs, diffResults;
    var correctCnt, testCnt, correctTagCnt, tagCnt;
    ...
    var api = {
        ...
        test: function () {
            ...
            testResults = _.map(actualData, function checkResult(actual, idx) {
                ...
                return clc.red.bold('X');
            });
            testCnt = _.size(testResults);

            // Semi-redundant analysis to get Tag precision, rather than sentence
            correctTagCnt = 0;
            tagCnt = 0;
            diffResults = _.map(expectedData, function (e, idx) {
                var diffInfo = diff(e, actualData[idx]);

                // Num words also give us num tags
                var numTags = _.size(e.split(' '));
                var incorrectSections = _.pluck(_.where(diffInfo, {correct: false}), 'str');
                var incorrectCnt = _.size(incorrectSections);

                tagCnt += numTags;
                correctTagCnt += (numTags - incorrectCnt);

                return _.pluck(diffInfo, 'str').join('');
            });
        ...
        printTest: function () {
            ...
            console.log('\nTest dataset:');
            printFixedWidth(
                ['', 'Test Sentence', 'Tagged Sentence (' + clc.green('expected') + ", " + clc.red('actual') + ')'],
                _.zip(testResults, testData, diffResults)
            );
            console.log("\nPrecision: " + (correctCnt / testCnt).toFixed(2) + " " +
                "(" + clc.green(correctCnt) + " of " + testCnt + ")");
            console.log("");
        ...
        getSentencePrecision: function () {
            return correctCnt / testCnt;
        },
        getTagPrecision: function () {
            return correctTagCnt / tagCnt;
        }
    };
